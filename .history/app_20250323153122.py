...
def download_financial_reports_em(stock_code):
    ...
    try:
        ...
        if balance_sheet is not None and not balance_sheet.empty:
            ...
            # 转换列名格式，与新浪财经的数据结构保持一致
            # 设置第一列为项目名称列
            # 为了兼容已有的分析函数，需要将数据转换为类似新浪财经的格式
            # 通常东方财富的数据第一列是REPORT_DATE（报告日期），我们需要将其移到列名
        ...
        if successful_downloads > 0:
            ...
            progress_bar.empty()
            status_text.empty()
            st.success(f"成功从东方财富下载了 {stock_code}({stock_name}) 的 {successful_downloads} 个财务报表")
            return report_data, stock_name
        else:
            status_text.empty()
            progress_bar.empty()
