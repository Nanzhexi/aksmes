...
def get_financial_metrics(income_statement):
    ...
    revenue_patterns = ['营业收入', '营业总收入', '主营业务收入', '总收入', '收入总计', '营业额', '营业总额', '营业收入总额', 'TOTAL_OPERATE_INCOME', 'OPERATE_INCOME']
    profit_patterns = ['净利润', '归属于母公司股东的净利润', '归属于上市公司股东的净利润', '利润总额', '净利', '利润', '净利润(含少数股东损益)', '净利润(扣除少数股东损益)', 'NETPROFIT', 'PARENT_NETPROFIT']
    ...
    for col in income_statement.columns[1:]:
        try:
            # 转换为数值
            revenue_value = pd.to_numeric(revenue_row[col].replace(',', ''), errors='coerce')
            profit_value = pd.to_numeric(profit_row[col].replace(',', ''), errors='coerce')
            
            # 检查是否为有效数值
            if not pd.isna(revenue_value) and not pd.isna(profit_value):
                # 使用日期作为键
                date = str(col).replace('.0', '')
                revenue_data[date] = revenue_value
                profit_data[date] = profit_value
        except:
            continue
    
    # 创建DataFrame
    if not revenue_data or not profit_data:
        st.warning("未能提取有效的财务数据")
        return None, None
    
    # 创建DataFrame
    financial_data = pd.DataFrame({
        '日期': list(revenue_data.keys()),
        '营业收入': list(revenue_data.values()),
        '净利润': list(profit_data.values())
    })
    ...
    return financial_data['营业收入'].tolist(), financial_data['净利润'].tolist()
