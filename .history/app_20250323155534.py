...
def get_financial_metrics(income_statement):
    ...
    # 打印利润表前5行供调试
    st.write("利润表结构预览:")
    st.dataframe(income_statement.head(10))  # 显示更多行
    ...
    revenue_patterns = ['营业收入', '营业总收入', '主营业务收入', '总收入', '收入总计', '营业额', '营业总额']
    ...
    # 取最近15年或全部数据（如果少于15年）
    max_years = 15
    st.write(f"共找到 {len(common_dates)} 个共同日期，将尝试使用最近 {max_years} 年的数据")
    common_dates = common_dates[:min(max_years, len(common_dates))]
