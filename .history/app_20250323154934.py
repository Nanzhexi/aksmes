...
def get_financial_ratios(balance_sheet, income_statement):
    ...
    try:
        # 首先转换为列表，然后再转换为字符串
        bs_items = balance_sheet.iloc[:, 0].tolist()
        balance_sheet.iloc[:, 0] = [str(x) for x in bs_items]
        
        is_items = income_statement.iloc[:, 0].tolist()
        income_statement.iloc[:, 0] = [str(x) for x in is_items]
    except Exception as e:
        st.error(f"转换列为字符串时出错: {e}")
        # 如果转换失败，尝试创建新的DataFrame
        try:
            # 处理资产负债表
            new_bs = pd.DataFrame()
    ...
    for date in common_dates:
        try:
            ...
            # 获取实际的列名
            bs_col = bs_orig_dates[bs_idx - 1]
            is_col = is_orig_dates[is_idx - 1]
            
            # 检查是否有有效数据
            if (pd.notna(asset_row[bs_col]) and pd.notna(equity_row[bs_col]) and 
                pd.notna(profit_row[is_col])):
                
                try:
                    total_asset = float(asset_row[bs_col])
                    net_equity = float(equity_row[bs_col])
                    net_profit = float(profit_row[is_col])
