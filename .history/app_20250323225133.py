...
def app():
    ...
    try:
        ...
        if revenue and profit and dates and len(revenue) > 0 and len(profit) > 0 and len(dates) > 0:
            ...
            # 添加手动数据可视化功能
            st.subheader("手动数据可视化")
            st.write("从利润表中选择数据行和日期列进行趋势图绘制")
            ...
            if not income_statement.empty:
                ...
                # 获取日期列（除了第一列外的所有列）
                date_cols = income_statement.columns[1:].tolist()
                
                # 允许用户选择项目行
                selected_items = []
                
                # 显示前20行项目供用户选择
                item_options = income_statement[first_col].head(20).tolist()
                selected_items = st.multiselect("选择要展示的财务项目", item_options)
                ...
                if selected_items:
                    ...
                    for item in selected_items:
                        # 查找对应行
                        item_row = income_statement[income_statement[first_col] == item]
                        ...
                        if not item_row.empty:
                            ...
                            # 如果有足够的数据点，添加到绘图数据中
                            if len(item_data) > 1:
                                item_series = pd.Series(item_data, name=item)
                                plot_data = pd.concat([plot_data, item_series.to_frame().T])
                    ...
                    if not plot_data.empty:
                        ...
                        # 绘制折线图
                        st.subheader(f"{stock_code} {stock_name} 财务趋势")
                        
                        # 格式化日期标签
                        plot_data_t.index = [str(idx).replace('.0', '') for idx in plot_data_t.index]
                        
                        # 不使用Streamlit的line_chart，避免Altair编码问题
                        # st.line_chart(plot_data_t)
                        
                        # 提供数据表格查看
                        with st.expander("查看图表数据"):
                            st.dataframe(plot_data_t)
                        ...
                        for col in plot_data_t.columns:
                            # 过滤有效数值
                            valid_data = plot_data_t[col].dropna()
                            if len(valid_data) > 1:
                                ax.plot(valid_data.index, valid_data.values, marker='o', linewidth=2, label=col)
                                
                                # 添加数据标签
                                for i, v in enumerate(valid_data):
                                    ax.text(list(valid_data.index)[i], v, f'{v/1e8:.2f}亿', ha='center', va='bottom')
                        
                        # 添加图表元素
                        plt.title(f'{stock_code} {stock_name} 财务指标趋势', fontsize=14)
                            ...
                            if len(valid_data) > 1:
                                ...
                                plotly_fig.add_trace(
                                    go.Scatter(
                                        ...
                                        name=col,
                                        text=[f'{v/1e8:.2f}亿' for v in valid_data.values],
                        ...
                        plotly_fig.update_layout(
                            title=f'{stock_code} {stock_name} 财务指标趋势 (交互式)',
                            xaxis_title='报告期',
                            yaxis_title='金额',
                            height=600,
                            hovermode='x unified',
                        ...
                        # 计算增长率
                        if st.checkbox("计算同比增长率"):
                            st.subheader("同比增长率计算")
                            
                            # 创建增长率表格
                            growth_data = pd.DataFrame()
                            
                            for item in selected_items:
                                if item in plot_data.index:
                                    item_values = []
                                    item_dates = []
                                    
                                    # 收集有效值
                                    for col in date_cols:
                                        if col in plot_data.columns and pd.notna(plot_data.loc[item, col]):
                                            item_values.append(plot_data.loc[item, col])
                                            item_dates.append(col)
                                    ...
                                    if len(item_values) >= 2:
                                        ...
                                        for i in range(1, len(item_values)):
                                            if item_values[i-1] != 0:  # 避免除以零
                                                growth_rate = (item_values[i] / item_values[i-1] - 1) * 100
                                                growth_rates.append(growth_rate)
                                                growth_dates.append(f"{item_dates[i-1]}→{item_dates[i]}")
                                        
                                        # 添加到增长率表格
                                        growth_df = pd.DataFrame({
                                            '期间': growth_dates,
                                            f'{item}_增长率(%)': growth_rates
                                        })
                                        
                                        st.write(f"{item} 的增长率:")
                                        st.dataframe(growth_df)
                                        
                                        # 计算平均增长率
                                        avg_growth = sum(growth_rates) / len(growth_rates)
                                        st.info(f"{item} 的平均增长率: {avg_growth:.2f}%")
                                        
                                        # 绘制增长率柱状图
                                        fig, ax = plt.subplots(figsize=(10, 4))
                                        ax.bar(growth_dates, growth_rates, alpha=0.7)
                                        ax.axhline(y=avg_growth, color='r', linestyle='--', alpha=0.5)
                                        plt.text(len(growth_dates)-1, avg_growth, f'平均: {avg_growth:.2f}%', color='r')
                                        plt.title(f'{item} 增长率趋势')
                                        plt.ylabel('增长率(%)')
                                        plt.xticks(rotation=45)
                                        plt.grid(alpha=0.3)
                                        plt.tight_layout()
                        ...
                        st.warning("无法从选定项目中提取有效的数值数据")
            else:
                st.warning("利润表为空，无法进行手动数据可视化")
    except Exception as e:
