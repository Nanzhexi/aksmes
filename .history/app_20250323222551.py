...
def app():
    ...
    try:
        ...
        # 如果成功加载，提取股票名称
        try:
            stock_name = balance_sheet.iloc[0, 0].split("资产负债表")[0].strip()
        except:
            try:
                stock_name = balance_sheet.columns[0].split("资产负债表")[0].strip()
            except:
                if debug_mode:
                    st.warning("无法从资产负债表中提取股票名称")
        ...
        # 如果成功提取财务指标，则绘制图表
        if revenue and profit and dates:
            ...
            st.warning("无法提取营业收入和净利润数据，请检查利润表格式")
