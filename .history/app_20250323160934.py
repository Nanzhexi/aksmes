...
def get_financial_metrics(income_statement):
    """从利润表中提取营业收入和净利润数据"""
    if income_statement is None or income_statement.empty or len(income_statement) < 1:
        st.warning("利润表数据为空")
        return None, None
    ...
    # 创建数据的深拷贝以避免警告，并确保第一列是字符串类型
    income_statement = income_statement.copy(deep=True)
    
    # 将整个DataFrame的列名转换为字符串类型
    income_statement.columns = [str(col) for col in income_statement.columns]
    
    # 检查第一列的数据类型
    st.write(f"第一列的数据类型: {income_statement.iloc[:, 0].dtype}")
    
    # 创建新DataFrame防止类型不兼容警告，一次性构建所有列
    data_dict = {}
    
    # 转换第一列为字符串
    data_dict[income_statement.columns[0]] = income_statement.iloc[:, 0].astype(str)
    
    # 复制其他列
    for col in income_statement.columns[1:]:
        data_dict[col] = income_statement[col]
    
    # 一次性创建新的DataFrame
    income_statement = pd.DataFrame(data_dict)
    ...
    revenue_patterns = ['营业收入', '营业总收入', '主营业务收入', '总收入', '收入总计', '营业额', '营业总额', '营业收入总额']
    ...
    # 查找匹配行
    revenue_rows = income_statement[income_statement.iloc[:, 0].str.contains(revenue_pattern, na=False, regex=True)]
    ...
    if revenue_rows.empty:
        ...
        revenue_rows = income_statement[income_statement.iloc[:, 0].str.contains('收入', na=False, regex=True)]
    ...
    for i, date in enumerate(col_dates):
        try:
            if i+1 < len(revenue_row) and pd.notna(revenue_row.iloc[i+1]) and revenue_row.iloc[i+1]:
                try:
                    # 尝试将值转换为浮点数
                    revenue_data[str(date)] = float(str(revenue_row.iloc[i+1]).replace(',', ''))
            ...
            if i+1 < len(profit_row) and pd.notna(profit_row.iloc[i+1]) and profit_row.iloc[i+1]:
                try:
                    # 尝试将值转换为浮点数
                    profit_data[str(date)] = float(str(profit_row.iloc[i+1]).replace(',', ''))
                except (ValueError, TypeError):
    ...
    # 使用所有可用的日期数据，不设置上限
    st.write(f"共找到 {len(common_dates)} 个共同日期，将使用所有日期")
...
def plot_financial_metrics(df, stock_code, stock_name):
    ...
    try:
        ...
        df_plot = df
        if len(df) > 15:
            st.info(f"数据点较多 ({len(df)}个)，图表将只显示最近15个数据点以确保清晰度")
        ...
        # 营业收入（左Y轴）
        revenue_data = df_plot['营业收入'].values / 100000000  # 转换为亿元
        ...
        if has_growth_data and len(df) >= 3:  # 至少需要3个数据点才能显示同比增长率
            ...
            # 排除第一行（没有增长率数据）
            growth_df = df_plot.iloc[1:].reset_index(drop=True)
            ...
            if growth_df['营业收入_同比增长'].notna().any() and growth_df['净利润_同比增长'].notna().any():
                ...
                # 使用不同颜色的柱状图展示增长率
                x = np.arange(len(growth_dates))
                width = 0.35
                
                revenue_bars = ax.bar(x - width/2, revenue_growth, width, alpha=0.7, color='steelblue', label='营业收入同比增长率')
                profit_bars = ax.bar(x + width/2, profit_growth, width, alpha=0.7, color='salmon', label='净利润同比增长率')
                
                # 在柱子上显示数值（如果数据点不多于10个）
                if len(growth_dates) <= 10:
                    for i, v in enumerate(revenue_growth):
                        if pd.notna(v):
                            ax.text(i - width/2, v, f'{v:.1f}%', ha='center', 
                ...
                # 设置x轴标签
                ax.set_xticks(x)
                ax.set_xticklabels(growth_dates, rotation=90 if len(growth_dates) > 5 else 45)
                
                # 添加标题和标签
                plt.title(f'{stock_code} ({stock_name}) 同比增长率对比')
                plt.xlabel('报告期')
                plt.ylabel('同比增长率 (%)')
                plt.grid(True, alpha=0.3)
                ...
                # 绘制趋势线
                ax.plot(growth_dates, revenue_growth, marker='o', color='steelblue', label='营业收入同比增长率')
                ax.plot(growth_dates, profit_growth, marker='s', color='salmon', label='净利润同比增长率')
                
                # 添加移动平均线（如果数据点足够）
                if len(growth_dates) >= 4:
                    # 计算3点移动平均
                    revenue_ma = np.convolve(revenue_growth, np.ones(3)/3, mode='valid')
                    profit_ma = np.convolve(profit_growth, np.ones(3)/3, mode='valid')
                    
                    # 截取对应的日期
                    ma_dates = growth_dates[1:-1]
                    
                    # 绘制移动平均线
                    ax.plot(ma_dates, revenue_ma, '--', color='blue', alpha=0.6, label='营业收入增长率3期移动平均')
                    ax.plot(ma_dates, profit_ma, '--', color='red', alpha=0.6, label='净利润增长率3期移动平均')
        ...
        # 添加扇形图，展示最近一期的收入与利润比例
        st.write("#### 最近一期收入与利润比例")
        
        # 获取最近一期数据
        latest_data = df.iloc[0]
        latest_date = latest_data['日期']
        
        # 计算毛利（假设为收入减去收入的70%作为成本）
        revenue = latest_data['营业收入'] / 100000000  # 转换为亿元
        profit = latest_data['净利润'] / 100000000  # 转换为亿元
        estimated_cost = revenue * 0.7  # 假设成本约占收入的70%
        estimated_gross_profit = revenue - estimated_cost  # 毛利 = 收入 - 成本
        ...
        # 第一个饼图：收入分解
        labels = ['净利润', '其他成本']
        sizes = [profit, revenue - profit]
        explode = (0.1, 0)  # 突出显示净利润部分
        
        ax1.pie(sizes, explode=explode, labels=labels, autopct='%1.1f%%',
                shadow=True, startangle=90, colors=['lightgreen', 'lightgray'])
        ax1.axis('equal')  # 确保饼图是圆的
        ax1.set_title(f'{latest_date} 营业收入构成 (总计: {revenue:.2f}亿元)')
        
        # 第二个饼图：成本分解（假设）
        if profit > 0:  # 只有在有利润时才显示
            labels = ['净利润', '估算毛利', '估算成本']
            sizes = [profit, estimated_gross_profit - profit, estimated_cost]
            explode = (0.1, 0, 0)  # 突出显示净利润部分
            
            ax2.pie(sizes, explode=explode, labels=labels, autopct='%1.1f%%',
                   shadow=True, startangle=90, colors=['lightgreen', 'skyblue', 'lightgray'])
            ax2.axis('equal')
            ax2.set_title(f'{latest_date} 成本与利润估算 (总计: {revenue:.2f}亿元)')
        else:
            ax2.text(0.5, 0.5, '净利润为负，无法显示比例', ha='center', va='center', fontsize=12)
            ax2.axis('off')
    ...
    try:
        ...
        try:
            ...
            if balance_sheet is None or balance_sheet.empty:
                try:
                    # 使用季度报表API
                    quarterly_bs = ak.stock_balance_sheet_by_report_em(symbol=stock_code)
            ...
            if balance_sheet is None or balance_sheet.empty:
                try:
                    quarterly_bs = ak.stock_balance_sheet_by_quarterly_em(symbol=stock_code)
                    ...
                    if 'REPORT_DATE' in quarterly_bs.columns:
                        ...
                        if not annual_bs.empty:
                            balance_sheet = annual_bs
                            st.success("使用季度报表API筛选得到年度资产负债表成功")
                except Exception as e:
        ...
        try:
            ...
            if income_statement is None or income_statement.empty:
                try:
                    # 使用季度报表API
                    quarterly_is = ak.stock_profit_sheet_by_report_em(symbol=stock_code)
            ...
            if income_statement is None or income_statement.empty:
                try:
                    quarterly_is = ak.stock_profit_sheet_by_quarterly_em(symbol=stock_code)
        ...
        try:
            ...
            if cash_flow is None or cash_flow.empty:
                try:
                    # 使用季度报表API
                    quarterly_cf = ak.stock_cash_flow_sheet_by_report_em(symbol=stock_code)
                    ...
                    if 'REPORT_DATE' in quarterly_cf.columns:
                        ...
                        if not annual_cf.empty:
                            cash_flow = annual_cf
                            st.success("使用季度报表筛选得到年度现金流量表成功")
                except Exception as e:
            ...
            if cash_flow is None or cash_flow.empty:
                try:
                    quarterly_cf = ak.stock_cash_flow_sheet_by_quarterly_em(symbol=stock_code)
        ...
        if not balance_sheet.empty:
            # 转换数据结构以与load_existing_reports兼容
            # 首先确保REPORT_DATE列存在
            if "REPORT_DATE" in balance_sheet.columns:
                ...
                try:
                    ...
                    # 保存原始数据作为备份
                    balance_sheet.to_csv(f"{reports_dir}/balance_sheet_raw.csv")
        ...
        if not income_statement.empty:
            ...
            if "REPORT_DATE" in income_statement.columns:
                try:
                    ...
                    # 保存原始数据作为备份
                    income_statement.to_csv(f"{reports_dir}/income_statement_raw.csv")
        ...
        if not cash_flow.empty:
            ...
            if "REPORT_DATE" in cash_flow.columns:
                try:
                    ...
                    # 保存原始数据作为备份
                    cash_flow.to_csv(f"{reports_dir}/cash_flow_raw.csv")
...
def app():
    """主函数"""
    st.set_page_config(page_title="A股财务分析", page_icon="📊", layout="wide")
    st.title("A股上市公司财务分析")
    ...
    if debug_mode:
        st.sidebar.info("调试模式已启用：将显示详细的数据处理过程和中间结果")
    
    # 股票代码输入
    stock_code = st.text_input("请输入股票代码（例如：600519）")
    
    if stock_code:
        # 确保股票代码格式正确（6位数字）
        if not re.match(r'^\d{6}$', stock_code):
            st.error("请输入正确的股票代码（6位数字）")
            return
        ...
        with st.spinner("加载财务报表..."):
            ...
            try:
                ...
                st.warning(f"获取股票名称时出错: {e}")
                stock_name = "未知"
            
            # 显示股票信息
            st.subheader(f"{stock_code} - {stock_name} 财务分析")
        
        # 提取财务指标和计算财务比率
        try:
            # 提取财务指标
            with st.spinner("提取财务指标..."):
                metrics_df, common_dates = get_financial_metrics(income_statement)
            
            # 如果成功提取财务指标，则绘制图表
            if metrics_df is not None and not metrics_df.empty:
                plot_financial_metrics(metrics_df, stock_code, stock_name)
            else:
                st.warning("未能提取有效的财务指标，无法绘制图表")
            ...
            if ratios_df is not None and not ratios_df.empty:
                ...
                st.warning("未能计算有效的财务比率，无法绘制财务比率图表")
                
        except Exception as e:
            st.error(f"处理财务数据时出错: {str(e)}")
            if debug_mode:
                import traceback
                st.error(traceback.format_exc())
